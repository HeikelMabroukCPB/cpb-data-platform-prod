...
PIPELINE_NAME = os.environ.get("PIPELINE_NAME", "salonkee_products")
...
TABLE_SCHEMA = [
    ...
    bigquery.SchemaField("purchase_price", "FLOAT64"),
    bigquery.SchemaField("stock", "INT64"),
    bigquery.SchemaField("published_in_store", "INT64"),
...
def transform_dataframe(df: pd.DataFrame, run_id: str) -> pd.DataFrame:
    ...
    df["ean"] = normalize_nullable_string(df["ean"])
    df["price"] = pd.to_numeric(df["price"], errors="coerce")
    df["purchase_price"] = pd.to_numeric(df["purchase_price"], errors="coerce")
    df["stock"] = pd.to_numeric(df["stock"], errors="coerce").astype("Int64")
    df["published_in_store"] = pd.to_numeric(
        df["published_in_store"], errors="coerce"
    ).astype("Int64")
