...
def log_public_ip() -> None:
    try:
        ...
        logger.info(
            "Cloud Run outbound public IP: %s",
            response.text.strip(),
        )

    except requests.exceptions.RequestException as exc:
        logger.warning(
            "Could not determine outbound public IP: %s",
            exc,
        )
...
def raise_for_api_status(response: requests.Response) -> None:
    ...
    if is_aws_waf_challenge(response):
        raise RuntimeError(
            "Salonkee request blocked by AWS WAF Human Verification. "
            "The request did not reach the API endpoint. "
            f"Status={response.status_code}, "
            f"URL={response.url}, "
            f"Content-Type={response.headers.get('Content-Type')}. "
            "Confirm that the Cloud Run job uses the static outbound IP "
            "and ask Salonkee to whitelist that IP or exclude the API "
            "route from the AWS WAF browser challenge."
        )
...
def fetch_data() -> pd.DataFrame:
    ...
    logger.info(
        "Fetching data from API | method=GET | url=%s | params=%s",
        API_URL,
        params,
    )
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    ...
                    f"body={get_response_body_preview(response)}"
                ) from exc
...
def run_etl():
    ...
    logger.info("Target raw table: %s", RAW_TABLE)

    try:
        validate_config()

        # Temporary diagnostic to verify Cloud NAT/static outbound IP.
        # Expected value: 104.155.100.39
        log_public_ip()
        ...
        return f"Pipeline failed: {str(exc)}", 500